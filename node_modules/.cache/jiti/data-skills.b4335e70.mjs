"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.skills = exports.locomotionTimeBySex = exports.femaleBwRatioOverride = void 0;
...
var _skillsExt = await jitiImport("./skills-ext"); // Complete Protean skill tree: every progression line of doc 01 §6, the three
...
const hold = (seconds, sets) => ({ kind: "hold", seconds, sets });const reps = (count, sets, perSide) => ({ kind: "reps", reps: count, sets, perSide
});
...
const e1rmRatio = (liftId, bwRatio) => ({ kind: "e1rm-ratio", liftId, bwRatio });
const timeUnder = (seconds, note) => ({ kind: "time", seconds, note });
const distanceOf = (meters, note) => ({ kind: "distance", meters, note });
/** NO_AUTO_UNLOCK (addendum-1 §0): user attestation is the only unlock path. */
const ATTESTED = { kind: "attested" };
...
const locomotionTimeBySex = exports.locomotionTimeBySex = Object.fromEntries(
 ...
 */
const femaleBwRatioOverride = exports.femaleBwRatioOverride = {
 ...
 */
function buildSkills(list, extensions) {
  const out = {};
  for (const n of list) {
...
const skills = exports.skills = buildSkills(nodeList, _skillsExt.sectorExtensions); /* v9-6635169ce16ce0bc */
